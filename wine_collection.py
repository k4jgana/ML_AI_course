...
if __name__ == '__main__':
    ...
    nula_klasa = [el for el in dataset if el[-1] == 0]
    kec_klasa = [el for el in dataset if el[-1] == 1]
    dva_klasa = [el for el in dataset if el[-1] == 2]
    x1 = 0.3
    x2 = 0.7
    train_set = nula_klasa[:int(x1 * len(nula_klasa))] + kec_klasa[:int(x1 * len(kec_klasa))] + dva_klasa[:int(
        x1 * len(dva_klasa))]
    train_set_x = [el[:-1] for el in train_set]
    train_set_y = [el[-1] for el in train_set]
    test_set = nula_klasa[int(x2 * len(nula_klasa)):] + kec_klasa[int(x2 * len(kec_klasa)):] + dva_klasa[int(x2 * len(
        dva_klasa)):]
    test_set_x = [el[:-1] for el in test_set]
    test_set_y = [el[-1] for el in test_set]

    clf1 = GaussianNB()
    clf1.fit(train_set_x, train_set_y)

    predictions = clf1.predict(test_set_x)
    corr = 0
    for true, pred in zip(test_set_y, predictions):
        if true == pred:
            corr += 1

    accu = corr / len(test_set)
    # print(f'NB ima tocnost {accu}')
    # Дрвo на одлука со записите од податочното множество во рангот од x1% до x2% од секоја класа.
    train_set_drva = nula_klasa[int(x1 * len(nula_klasa)):int(x2 * len(nula_klasa))] + kec_klasa[int(x1 * len(kec_klasa)):int(x2 * len(kec_klasa))] + dva_klasa[int(x1 * len(dva_klasa)):int(x2 * len(dva_klasa))]
    train_set_drva_x = [el[:-1] for el in train_set_drva]
    train_set_drva_y = [el[-1] for el in train_set_drva]

    clf2 = DecisionTreeClassifier()

    clf2.fit(train_set_drva_x, train_set_drva_y)

    predictions = clf2.predict(test_set_x)
    corr = 0
    for true, pred in zip(test_set_y, predictions):
        if true == pred:
            corr += 1

    accu2 = corr / len(test_set)

    # print(f'DecisionTree ima tocnost {accu2}')

    train_set_forrest = nula_klasa[:int(x2 * len(nula_klasa))] + kec_klasa[:int(x2 * len(kec_klasa))] + dva_klasa[:int(
        x2 * len(dva_klasa))]
    train_set_forrest_x = [el[:-1] for el in train_set_forrest]
    train_set_forrest_y = [el[-1] for el in train_set_forrest]

    clf3 = RandomForestClassifier(n_estimators=3)

    clf3.fit(train_set_forrest_x, train_set_forrest_y)

    predictions = clf3.predict(test_set_x)
    corr = 0
    for true, pred in zip(test_set_y, predictions):
        if true == pred:
            corr += 1

    accu3 = corr / len(test_set)

    # print(f'RandomForrest ima tocnost {accu2}')

    true_acc = 0

    # Потребно е да ја пресметате и испечатите точноста на колекцијата од класификатори.
    # Еден запис се смета за точно
    # класифициран доколку класата во која припаѓа е точно предвидена од барем 2 класификатори.

    # for entry, result in zip(test_x, test_y):
    #     count = 0
    #     for classifier in [NB_classifier, DT_classifier, RF_classifier]:
    #         if classifier.predict([entry])[0] == result:
    #             count += 1
    #     if count >= 2:
    #         acc += 1

    for pred, true in zip(test_set_x, test_set_y):
        counter = 0
        for clfs in [clf1, clf2, clf3]:
            if clfs.predict([pred])[0] == true:
                counter += 1
        if counter > 1:
            true_acc += 1

    print(f'Tochnost: {true_acc / len(test_set)}')
